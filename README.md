...
본 프로젝트의 목적은 **저해상도(64×64) 조류 이미지 환경에서 Knowledge Distillation(KD)가 분류 성능 향상에 얼마나 효과적인지**를 검증하는 것이다.
...
단순 Hard-label 학습 대비 성능 차이를 분석한다.

---

## 1. Motivation

저해상도 이미지에서는 다음과 같은 문제가 발생한다.
...
LR 환경에서도 Student 모델이 더 풍부한 표현을 학습할 수 있는지를 확인한다.

---

## 2. Dataset

본 프로젝트는 DACON의 공개 경진대회 데이터를 기반으로 한다.  
데이터는 저해상도 조류 이미지와 해당 이미지의 x4 Upscale 버전으로 구성되어 있다.
...
Teacher 모델은 사전 학습 후 **freeze**하여 Student 학습 동안 업데이트하지 않는다.
...
환경 및 의존성 정보는 `requirements.txt`에 별도로 정리되어 있다.

공정한 비교를 위해 baseline Student와 KD Student는
동일한 backbone과 하이퍼파라미터 설정으로 학습하였다.
두 모델 간의 성능 차이는 Knowledge Distillation 적용 여부에서만 발생하도록 설계하였다.
...
LR Hard-label baseline 대비 **+1.58%p 성능 향상**을 보였다.
...
**저해상도 입력 조건에서 KD 기법의 효과를 체감하고 분석하는 데 목적**을 두었다.  
